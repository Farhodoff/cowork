...
import { parseSplitCommand } from "../services/geminiSplitService.js";

/**
 * @swagger
 * /sessions/ai-split:
 *   post:
 *     summary: Split receipt items automatically using AI Voice/Text command
 *     tags: [Sessions]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *           schema:
 *             type: object
 *             required: [prompt, participants, items]
 *             properties:
 *               prompt:
 *                 type: string
 *                 example: "Ali ate kabob, I drank tea"
 *               participants:
 *                 type: array
 *                 items:
 *                   type: object
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const { prompt, participants, items, audio } = req.body || {};
      if ((!prompt || typeof prompt !== "string") && !audio) {
        return res.status(400).json({ error: "prompt string or audio object required" });
      }
      if (!Array.isArray(participants) || !Array.isArray(items)) {
        return res.status(400).json({ error: "participants and items arrays required" });
      }

      const result = await parseSplitCommand({
        prompt: prompt || "",
        audio,
        participants,
        items,
      ...
      console.error("POST /sessions/ai-split error:", err);
      const errorMessage = err instanceof Error ? err.message : "Unknown error occurred";
      return res.status(500).json({ error: "AI Split failed", details: errorMessage });
    }
  }
);
...
router.post("/", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Log activity if session belongs to a group
    if (created.groupId) {
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Log activity if session belongs to a group
      const sessionForActivity = await prisma.session.findUnique({
        where: { id: Number(sessionId) },
        select: { groupId: true },
      });
      if (sessionForActivity?.groupId) {
        await prisma.groupActivity.create({
          data: {
            groupId: sessionForActivity.groupId,
            actorId: req.user.id,
