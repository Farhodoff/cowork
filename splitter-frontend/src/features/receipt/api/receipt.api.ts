...
export const ReceiptApi = {
  ...
  async aiSplit(payload: AiSplitRequest): Promise<AiSplitResponse> {
    try {
      const { data } = await apiClient.post<AiSplitResponse>('/sessions/ai-split', payload);
      return data;
    } catch (error) {
      console.error('[API] Error (aiSplit):', error);
      throw normalizeError(error);
    }
  },
};
