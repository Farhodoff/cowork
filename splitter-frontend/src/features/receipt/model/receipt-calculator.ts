...
const ensureMode = (item: CalculatorItem): SplitMode =>
  ...
  (item.splitMode === undefined && item.quantity > 1)
    ? "count"
    : "equal";
...
export const buildLocalFinalization = (
  ...
  const participantTotals = participants.reduce<Record<string, number>>(
    (acc, participant) => {
  ...
  for (const item of items) {
    ...
    if (shareCount === 0) {
      // No participants assigned for equal split; skip this item.
      continue;
    }
  ...
  const totalsByParticipant: FinalizeTotalsByParticipant[] = participants.map(
    (participant) => ({
